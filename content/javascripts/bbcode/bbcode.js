(function (e) {
	...
	e.fn.BBCode = function (options) {
		...
		var rowCount	=	0;
		var default_url ="/content/javascripts/bbcode"; 
		
		$.get(default_url + '/themes/template.tpl', function(data) {
			that.remove();
			parent.append(data);
		...
		$.each(settings, function( key, value ) {
			setTimeout(
				function(){
					if(value){
						$.get(default_url + '/themes/toolbar-'+ key +'.tpl').done(function(data) {
		...
		$(document).on('click','button[data-rule="table-panel"]',function(){
			...
			//Call New Table
			$.get(default_url + '/themes/table-template.tpl', function(data) {
				html=data;
					...
					switch(type){
						case "file":
							if(data.status == 'ok'){
								...
								$.each(data.result, function(index, element) {
									liHTML += '<li><a href="#" data-rule="' + type + '" rel="' + element.file_path + '"><span class="modal-display-name">' + element.name  + '</span><br/><span class="modal-file-name">'+ element.date_time + '</span></a></li>';
								});
							}
						...
						case "image":
							if(data.status == 'ok'){
								var liHTML='';
								$.each(data.result, function(index, element) {
									liHTML += '<img data-rule="' + type + '" rel="' + element.file_path + '" src="/content/uploads/' + element.file_path + '" alt="'+ element.name +'">';
								});
